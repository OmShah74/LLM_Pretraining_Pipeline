...
from src.train.common import build_sft_dataloaders, load_tokenizer, setup_run, train_language_model
from src.utils.config import load_full_config


def main() -> None:
    parser = argparse.ArgumentParser(description="Run SFT.")
    ...
    train_loader, eval_loader = build_sft_dataloaders(config["data"], tokenizer, config["train"])
